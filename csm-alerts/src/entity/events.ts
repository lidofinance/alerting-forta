...
export function handleEventsOfNotice(txEvent: TransactionEvent, eventsOfNotice: EventOfNotice[]): Finding[] {
  const out: Finding[] = []
  ...
  for (const log of txEvent.logs) {
    for (const eventInfo of eventsOfNotice) {
      ...
      const parser = new ethers.Interface(typeof eventInfo.abi === 'string' ? [eventInfo.abi] : eventInfo.abi)
