import { BlockEvent, Finding, FindingSeverity, FindingType, TransactionEvent } from '@fortanetwork/forta-bot'
...
export function sourceFromEvent(event: TransactionEvent | BlockEvent): FindingSource {
  ...
  if ('transaction' in event) source.transactions = [{ chainId: event.chainId, hash: event.transaction.hash }]
...
export function failedTxAlert(txEvent: TransactionEvent, description: string, reason: string): Finding {
  return Finding.fromObject({
