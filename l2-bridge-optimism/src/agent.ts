...
const subAgents: SubAgent[] = [agentGov, agentProxy, agentBridge, agentWithdrawals]
...
const handleBlock: HandleBlock = async (blockEvent: BlockEvent): Promise<Finding[]> => {
  ...
  for (let i = 0; i < runs.length; i++) {
    const r = runs[i] as PromiseRejectedResult
    if (r.status == 'rejected') {
      blockFindings.push(errorToFinding(r.reason, subAgents[i], 'handleBlock'))
    }
  }
...
const handleTransaction: HandleTransaction = async (txEvent: TransactionEvent) => {
  ...
  for (let i = 0; i < runs.length; i++) {
    const r = runs[i] as PromiseRejectedResult
    if (r.status == 'rejected') {
      txFindings.push(errorToFinding(r.reason, subAgents[i], 'handleBlock'))
    }
  }
