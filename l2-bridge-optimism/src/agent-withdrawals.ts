...
export async function initialize(currentBlock: number): Promise<{ [key: string]: string }> {
  ...
  const l2Bridge = new ethers.Contract(L2_ERC20_TOKEN_GATEWAY, L2_BRIDGE_ABI, ethersProvider)
  ...
  const withdrawEvents = await l2Bridge.queryFilter(withdrawalInitiatedFilter, pastBlock, currentBlock - 1)
  await Promise.all(
  ...
  withdrawalsCache.forEach((x: IWithdrawalRecord) => (withdrawalsSum = withdrawalsSum.plus(x.amount)))
  return {
...
async function handleToManyWithdrawals(blockEvent: BlockEvent, findings: Finding[]) {
  const now = blockEvent.block.timestamp
  // remove withdrawals records older than MAX_WITHDRAWALS_WINDOW
  withdrawalsCache = withdrawalsCache.filter((x: IWithdrawalRecord) => x.time > now - MAX_WITHDRAWALS_WINDOW)
  let withdrawalsSum = new BigNumber(0)
  withdrawalsCache.forEach((x: IWithdrawalRecord) => (withdrawalsSum = withdrawalsSum.plus(x.amount)))
  const period =
  ...
  if (
    ...
    findings.push(
      Finding.fromObject({
        name: `⚠️ Optimism: Huge withdrawals during the last ` + `${Math.floor(period / (60 * 60))} hour(s)`,
        description:
    ...
    withdrawalsCache = withdrawalsCache.filter((x: IWithdrawalRecord) => x.time > lastReportedToManyWithdrawals)
  }
}

export async function handleTransaction(txEvent: TransactionEvent) {
  handleWithdrawalEvent(txEvent)

  return []
}

function handleWithdrawalEvent(txEvent: TransactionEvent) {
  if (L2_ERC20_TOKEN_GATEWAY in txEvent.addresses) {
    const events = txEvent.filterLog(WITHDRAWAL_INITIATED_EVENT, L2_ERC20_TOKEN_GATEWAY)
