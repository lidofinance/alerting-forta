import { ethers, BlockEvent, TransactionEvent, Finding, FindingType, FindingSeverity } from 'forta-agent'
import { PROXY_ADMIN_EVENTS, LIDO_PROXY_CONTRACTS, LidoProxy } from './constants'
...
async function getProxyImpl(proxyInfo: LidoProxy, blockNumber: number) {
  ...
  const proxy = new ethers.Contract(proxyInfo.address, proxyInfo.shortABI, ethersProvider)
  return (await proxy.functions[implFunc]({ blockTag: blockNumber }))[0]
}

async function handleProxyImplementationChanges(blockEvent: BlockEvent, findings: Finding[]) {
...
async function handleProxyAdminChanges(blockEvent: BlockEvent, findings: Finding[]) {
  if (blockEvent.blockNumber % BLOCK_INTERVAL == 0) {
    await Promise.all(
      LIDO_PROXY_CONTRACTS.map(async (proxyInfo: LidoProxy) => {
