...
export class MonitorWithdrawals {
  ...
  private readonly optimismL2TokenGateway: string
  ...
  constructor(withdrawalsClient: IMonitorWithdrawalsClient, optimismL2TokenGateway: string, logger: Logger) {
    this.withdrawalsClient = withdrawalsClient
    this.optimismL2TokenGateway = optimismL2TokenGateway
    this.logger = logger
  }
  ...
  public handleL2Block(l2BlockDto: BlockDto): Finding[] {
    ...
    if (
      ...
      f.setName(`⚠️ Optimism: Huge withdrawals during the last ` + `${Math.floor(period / (60 * 60))} hour(s)`)
      f.setDescription(
  ...
  public handleTransaction(txDto: TransactionDto): WithdrawalRecord | null {
    for (const log of txDto.logs) {
      if (log.address.toLowerCase() !== this.optimismL2TokenGateway.toLowerCase()) {
        continue
      }
      ...
      try {
        ...
        const out: WithdrawalRecord = {
          ...
          amount: new BigNumber(String(logDesc.args._amount)),
        }
