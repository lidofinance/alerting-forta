...
import { IZkSyncClient } from './zksync_client'
...
export class ZkSyncBlockClient {
  private provider: IZkSyncClient
  private logger: Logger
  private cachedBlockDto: BlockDto | undefined = undefined

  constructor(provider: IZkSyncClient, logger: Logger) {
  ...
  public async getL2Blocks(): Promise<E.Either<Finding, BlockDto[]>> {
    ...
    this.logger.info(elapsedTime(ZkSyncBlockClient.name + '.' + this.getL2Blocks.name, start))
  ...
  public async getL2Logs(workingBlocks: BlockDto[]): Promise<E.Either<Finding, Log[]>> {
    ...
    this.logger.info(elapsedTime(ZkSyncBlockClient.name + '.' + this.getL2Logs.name, start))
    ...
    if (this.cachedBlockDto === undefined) {
      ...
      if (E.isLeft(l2Block)) {
        return E.left(
          networkAlert(
            l2Block.left,
            `Error in ${ZkSyncBlockClient.name}.${this.getL2Blocks.name}:21`,
      ...
      if (E.isLeft(latestL2Block)) {
        ...
        return E.left(
          networkAlert(
            latestL2Block.left,
            `Error in ${ZkSyncBlockClient.name}.${this.getL2Blocks.name}:39`,
            `Could not call zkSyncProvider.getLatestBlock`,
            0,
          ),
        )
  ...
  private async fetchL2Logs(workingL2Blocks: BlockDto[]): Promise<E.Either<Finding, Log[]>> {
    ...
    if (E.isLeft(l2Logs)) {
      return E.left(
        networkAlert(
          l2Logs.left,
          `Error in ${ZkSyncBlockClient.name}.${this.getL2Logs.name}:76`,
