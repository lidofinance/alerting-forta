...
export abstract class IBaseProvider {
  ...
  public abstract getStartedBlockForApp(
    argv: string[],
...
export class BaseProvider implements IBaseProvider {
  ...
  public async getStartedBlockForApp(
    argv: string[],
  ): Promise<E.Either<Error, number>> {
    ...
    if (argv.includes('--block')) {
      ...
      if (!tx.blockNumber) {
        return E.left(
          Error(`Transaction ${txHash} was not yet included into block`),
        )
      }
