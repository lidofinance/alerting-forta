...
const subAgents: SubAgent[] = [agentEthereum, agentPolygon, agentArbitrum, agentOptimism, agentMoonbeam, agentMoonriver]
...
const handleBlock: HandleBlock = async (blockEvent: BlockEvent): Promise<Finding[]> => {
  ...
  runs.forEach((r: PromiseSettledResult<unknown>, index: number) => {
    if (r.status == 'rejected') {
      blockFindings.push(errorToFinding(r.reason, subAgents[index], 'handleBlock'))
    }
  })
...
const handleTransaction: HandleTransaction = async (txEvent: TransactionEvent) => {
  ...
  runs.forEach((r: PromiseSettledResult<unknown>, index: number) => {
    if (r.status == 'rejected') {
      txFindings.push(errorToFinding(r.reason, subAgents[index], 'handleBlock'))
    }
  })
