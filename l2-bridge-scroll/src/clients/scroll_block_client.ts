...
import { IScrollClient } from './scroll_client'
...
export class ScrollBlockClient {
  private provider: IScrollClient
  private logger: Logger
  private cachedBlockDto: BlockDto | undefined = undefined

  constructor(provider: IScrollClient, logger: Logger) {
  ...
  public async getL2Blocks(): Promise<E.Either<Finding, BlockDto[]>> {
    ...
    this.logger.info(elapsedTime(ScrollBlockClient.name + '.' + this.getL2Blocks.name, start))
  ...
  public async getL2Logs(workingBlocks: BlockDto[]): Promise<E.Either<Finding, Log[]>> {
    ...
    this.logger.info(elapsedTime(ScrollBlockClient.name + '.' + this.getL2Logs.name, start))
    ...
    if (this.cachedBlockDto === undefined) {
      ...
      if (E.isLeft(l2Block)) {
        return E.left(
          networkAlert(
            l2Block.left,
            `Error in ${ScrollBlockClient.name}.${this.getL2Blocks.name}:21`,
      ...
      if (E.isLeft(latestL2Block)) {
        ...
        return E.left(
          networkAlert(
            latestL2Block.left,
            `Error in ${ScrollBlockClient.name}.${this.getL2Blocks.name}:39`,
            `Could not call scrollProvider.getLatestBlock`,
            0,
          ),
        )
  ...
  private async fetchL2Logs(workingL2Blocks: BlockDto[]): Promise<E.Either<Finding, Log[]>> {
    ...
    if (E.isLeft(l2Logs)) {
      return E.left(
        networkAlert(
          l2Logs.left,
          `Error in ${ScrollBlockClient.name}.${this.getL2Logs.name}:76`,
