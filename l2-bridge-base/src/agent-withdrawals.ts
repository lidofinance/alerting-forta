...
export async function initialize(
    ...
    console.log(`[${name}]`);
    const myCurrentBlock = await baseProvider.getBlockNumber()
    const now = (await baseProvider.getBlock(myCurrentBlock)).timestamp
    ...
    const pastBlock = myCurrentBlock - Math.ceil(MAX_WITHDRAWALS_WINDOW / 13);
    const withdrawEvents = await l2Bridge.queryFilter(
        withdrawalInitiatedFilter,
        pastBlock, myCurrentBlock - 1,
    );
