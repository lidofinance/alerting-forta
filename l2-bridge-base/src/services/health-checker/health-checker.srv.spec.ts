...
describe('HealthChecker', () => {
  const networkFindings = [networkAlert(new Error('Some err'), 'err name', 'err desc', 100_000)]
  ...
  test('should become unhealthy', async () => {
    ...
    const findings3 = [
      networkAlert(new Error('Some err'), 'err name', 'err desc', 100_1001),
      networkAlert(new Error('Some err'), 'err name', 'err desc', 100_1002),
      networkAlert(new Error('Some err'), 'err name', 'err desc', 100_1003),
    ]
