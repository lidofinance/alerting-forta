...
async function handleAstEthSupply(blockEvent: BlockEvent, findings: Finding[]) {
  ...
  if (lastReportedAstEthSupply + REPORT_WINDOW < now) {
    ...
    if (difference.isGreaterThanOrEqualTo(ASTETH_GWEI_DIFFERENCE_THRESHOLD)) {
      findings.push(
        Finding.fromObject({
...
async function handleStableStEthSupply(
  ...
  if (lastReportedStableStEthSupply + REPORT_WINDOW < now) {
    ...
    if (stableDebtStEthTotalSupply.isLessThanOrEqualTo(0)) {
      findings.push(
        Finding.fromObject({
          name: "stableDebtStETH totalSupply is 0",
          description: `stableDebtStETH totalSupply is 0`,
...
async function handleVariableStEthSupply(
  ...
  if (lastReportedVariableStEthSupply + REPORT_WINDOW < now) {
    ...
    if (variableDebtStEthTotalSupply.isLessThanOrEqualTo(0)) {
      findings.push(
        Finding.fromObject({
          name: "variableDebtStETH totalSupply is 0",
          description: `variableDebtStETH totalSupply is 0`,
